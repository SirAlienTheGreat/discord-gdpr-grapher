...
fn main() {
    ...
    for path in paths { // Convert path (:ReadDir) to :String
        if !path.as_ref().unwrap().path().display().to_string().contains("index"){
            str_path.push([path.unwrap().path().display().to_string(),"/messages.json".to_string()].join(""))
        }
    ...
    for path in str_path { // For every messages file:
        ...
        //let message_file: Vec<Message> = serde_json::from_str("[{\"ID\": 730916053596766238, \"Timestamp\": \"2020-07-09 22:39:26\", \"Contents\": \"I'm an Alien too\", \"Attachments\": \"\"}]").unwrap();

        let message_file: Vec<Message> = serde_json::from_str(&randomstring).unwrap();

        for message in message_file{
            dates.push(message.Timestamp);
            messages.push(message.Contents);
        }
